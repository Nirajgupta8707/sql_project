...
select  origin_city_name , dest_city_name ,  sum(passengers) as total_passengers from flight
group by 1,2
...
-- for routes
select origin_city_name ,  dest_city_name , avg(passengers) as average_passengers from flight
group by 1,2
...
select origin_airport_id , avg(passengers) average_passengers  from flight
group by 1
order by average_passengers desc
limit 5;
...
select ORIGIN_CITY_NAME, DEST_CITY_NAME , count(airline_id) as total_flights from flight
group by 1 ,2
...
select * from flight;
 ...
 select airline_id ,  max(passengers) as Total_passengers from flight
 group by 1
 having Total_passengers >0),
 
 seat_utilization as (
 select  fi.airline_id , fi.passengers * 100/sc.Total_passengers  as seat_utilization from flight fi 
